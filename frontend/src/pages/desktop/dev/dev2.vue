...
const columns = [
  ...
  },
  {
    ...
    align: 'center',
    label: 'Calories',
    field: (row: { calories: any }) => row.calories,
    format: (val: any) => {
      //console.log(val);
      return `${val}`
    },
  ...
  { name: 'fat', label: 'Fat (g)', field: 'fat', sortable: true, translate: true },
...
function fetchFromServer(startRow: number | undefined, count: any, filter: string, sortBy: string, descending: any) {
  const data = filter ? originalRows.filter((row) => row.name.includes(filter)) : originalRows.slice()
    ...
    const sortFn =
      sortBy === 'desc'
        ? descending
          ...
          : (a: any, b: any) => (a.name > b.name ? 1 : a.name < b.name ? -1 : 0)
        : descending
        ? (a: any, b: any) => parseInt(b[sortBy]) - parseInt(a[sortBy])
        : (a: any, b: any) => parseInt(a[sortBy]) - parseInt(b[sortBy])
    data.sort(sortFn)
  }
