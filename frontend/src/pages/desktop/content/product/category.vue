...
import { DELETE_PRODUCT_CATEGORY, INSERT_PRODUCT_CATEGORY, SELECT_PARTNER_LIST, SELECT_PRODUCT_CATEGORY_EXISTS, UPDATE_PRODUCT_CATEGORY } from 'src/api/admin-api'
...
async function isCodeCheck(code: string) {
  return await SELECT_PRODUCT_CATEGORY_EXISTS(code)
}

const saveItem = async () => {
  const data: ProductCategory = Object.assign({}, selectedItem.value, { CM_IDS: qtable.value?.selected?.map((d) => d.CM_ID).join(';') })
  console.log(data)
  let result: number
  if (isUpdate.value) {
    result = await UPDATE_PRODUCT_CATEGORY(data)
  } else {
    result = await INSERT_PRODUCT_CATEGORY(data)
  }
  ...
  dialog.confirmMessage({
    ...
    callback: async (state: boolean) => {
      if (state) {
        ...
        let result: number = await DELETE_PRODUCT_CATEGORY(data)
        await dialog.message({
          message: result != -1 ? $t(i18n.common.BeProcessed) : $t(i18n.common.FailedBeProcessed),
          callback: () => {
            if (result != -1) {
